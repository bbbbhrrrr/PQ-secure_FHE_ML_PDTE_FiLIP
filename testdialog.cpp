...
void TestDialog::on_encryptButton_clicked()
{
    QString input_csv_path = ui->encryptInputLineEdit->text();
    QString output_csv_path = ui->encryptOutputLineEdit->text();

    if (input_csv_path.isEmpty() || output_csv_path.isEmpty()) {
        QMessageBox::warning(this, "路径错误", "请输入有效的输入文件路径和输出文件路径。");
    }
    ...
    // 调用 filip_enc 函数进行加密
    filip_enc(input, output, 10);

    ui->encryptOutputTextEdit->append(QString::fromStdString(ui->encryptOutputTextEdit->getBufferContent()));
    QMessageBox::information(this, "完成", "加密已完成，结果已保存到输出文件。");
}
...
{
    // 模型1的实现
    model_file = "/home/bbbbhrrrr/qt_gold/gold/src/cpp_pdte_transciphering/data/breast_11bits/model.json";
    num_bits = 11;
...
{
    // 模型2的实现
    model_file = "/home/bbbbhrrrr/qt_gold/gold/decision_tree.json";
    num_bits = 10;
    runTests(input_csv_path, output_csv_path, model_file, num_bits);
}
...
{
    // 模型3的实现
    model_file = "/home/bbbbhrrrr/qt_gold/gold/src/cpp_pdte_transciphering/data/phoneme_10bits/model.json";
    num_bits = 10;
    runTests(input_csv_path, output_csv_path, model_file, num_bits);
}
...
{
    // 模型4的实现
    model_file = "/home/bbbbhrrrr/qt_gold/gold/src/cpp_pdte_transciphering/data/spam_11bits/model.json";
    num_bits = 11;
    runTests(input_csv_path, output_csv_path, model_file, num_bits);
}
...
void TestDialog::filip_enc(const std::string& input_csv_path, const std::string& output_csv_path, int num_bits)
{
    // 打开输入文件
    std::ifstream input_file(input_csv_path);
    if (!input_file.is_open()) {
    ...
    while (std::getline(input_file, line)) {
        ...
        std::vector<std::vector<int>> enc_bits(plain_features.size()); // enc_bits[i] == i-th 特征的加密位
